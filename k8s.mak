...
loader: dynamodb-init $(LOG_DIR)/loader.repo.log cluster/loader.yaml
	...
	tools/build-configmap.sh gatling/resources/music.csv cluster/playlist-header.yaml | kubectl -n $(APP_NS) apply -f -
...
$(LOG_DIR)/dynamodb-init.log: cluster/cloudformationdynamodb.json
	...
	$(AWS) cloudformation create-stack --stack-name db-jinghan-yu --template-body file://$< || true | tee $(LOG_DIR)/dynamodb-init.log
...
#
dynamodb-clean:
	$(AWS) cloudformation delete-stack --stack-name db-jinghan-yu || true | tee $(LOG_DIR)/dynamodb-clean.log
...
cr: registry-login
	...
	$(DK) push $(CREG)/$(REGID)/cmpt756s3:$(S3_VER) | tee $(LOG_DIR)/s3.repo.log
	$(DK) push $(CREG)/$(REGID)/cmpt756db:$(APP_VER_TAG) | tee $(LOG_DIR)/db.repo.log
